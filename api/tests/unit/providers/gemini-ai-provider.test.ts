...
describe('GeminiAiProvider (Function Calling)', () => {
  ...
  it('injects current date and brazil timezone instruction in system context', async () => {
    ...
    expect(firstContentPart).toContain('add_shopping_list_item')
    expect(firstContentPart).toContain('get_shopping_list')
    expect(firstContentPart).toContain('clear_shopping_list')
    expect(firstContentPart).not.toContain('get_current_diet')
    expect(firstContentPart).not.toContain('search_food_nutrition')
    expect(firstContentPart).not.toContain('suggest_food_swap')
    expect(firstContentPart).not.toContain('update_diet_meal_option')
    expect(firstContentPart).not.toContain('DIRETRIZES DE FORMATACAO VISUAL')
    expect(firstContentPart).not.toContain('interface premium')
    expect(firstContentPart).not.toContain('dashboard completo')
    expect(firstContentPart).toContain('dashboard simples')
    expect(firstContentPart).toContain('Nao use ---')
    expect(firstContentPart).toContain('Nao use bullets com *')
    expect(firstContentPart).toContain('linha separadora final')
    expect(firstContentPart).toContain('Total: R$')
  })
})
